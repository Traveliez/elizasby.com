...
session_start();
    ...
    $tampil_xmrebate = query("SELECT * FROM data_xmrebate ORDER BY id DESC");
?>
                        ...
                        <div class="body">
                            <div class="table-responsive">
                                <table class="table table-bordered table-striped table-hover js-basic-example dataTable">
                                    ...
                                    <tbody>
                                    <?php $i = 1; ?>
                                    <?php foreach ($tampil_xmrebate as $row ) : ?> 
                                        <tr>
                                            <td>
                                                <?php 
                                                    $t = $row['tanggal'];
                                            ...
                                            $query = query("SELECT * FROM data_xmrebate WHERE id='$idbukti'");
                                            ...
                                            <td class="actions">
                                                <a href="add/xmrebate/accept.php?id=<?= $row["id"] ?>" >
                                                    <span class="badge bg-cyan"><i class="material-icons">done</i></span> 
                                                </a> ||
                                                <a href="add/xmrebate/cancel.php?id=<?= $row["id"] ?>" onClick="if(confirm('Apakah anda yakin ?')){return true}else{return false}" >
                                                    <span class="badge bg-orange"><i class="material-icons">close</i></span>
                                                </a> ||
                                                <a href="add/xmrebate/delete.php?id=<?= $row["id"] ?>" onClick="if(confirm('Apakah anda yakin ingin menghapusnya ?')){return true}else{return false}" >
