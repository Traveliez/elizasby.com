...
session_start();
    ...
    $tampil_welltrade = query("SELECT * FROM data_welltrade ORDER BY id DESC");
?>
                        ...
                        <div class="body">
                            <div class="table-responsive">
                                <table class="table table-bordered table-striped table-hover js-basic-example dataTable">
                                    ...
                                    <tbody>
                                    <?php $i = 1; ?>
                                    <?php foreach ($tampil_welltrade as $row ) : ?> 
                                        <tr>
                                            <td>
                                                <?php 
                                                    $t = $row['tanggal'];
                                            ...
                                            $query = query("SELECT * FROM data_welltrade WHERE id='$idbukti'");
                                            ...
                                            <td class="actions">
                                                <a href="add/welltrade/accept.php?id=<?= $row["id"] ?>" >
                                                    <span class="badge bg-cyan"><i class="material-icons">done</i></span> 
                                                </a> ||
                                                <a href="add/welltrade/cancel.php?id=<?= $row["id"] ?>" onClick="if(confirm('Apakah anda yakin ?')){return true}else{return false}" >
                                                    <span class="badge bg-orange"><i class="material-icons">close</i></span>
                                                </a> ||
                                                <a href="add/welltrade/delete.php?id=<?= $row["id"] ?>" onClick="if(confirm('Apakah anda yakin ingin menghapusnya ?')){return true}else{return false}" >
