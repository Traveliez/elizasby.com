...
if (isset($_FILES['file']['name']) && in_array($_FILES['file']['type'], $file_mimes)) {
    ...
    foreach ($arrData as $index => $data) {
        ...
        if($email_rebate[0]['email'] == $email_saku[0]['email']){
            ...
            echo "
                <script>
                    ...
                    document.location.href = 'tampilan-xmrebate.php';
                </script>
                ";
        }
        ...
        if (isset($bank[0])) {
            ...
            $xx = "'" . implode("','", $data) . "'";
            $sql = "INSERT INTO data_xmrebate (tanggal, periode, no_akun, email, auto_rebate, rebate_dollar, rebate_rupiah, lot, bank, norek) VALUES ('$tanggal', $xx, '" . $bank[0]['bank'] . "', '" . $bank[0]['norek'] . "')";
            $insert = mysqli_query($conn, $sql);
            if ($insert === TRUE) {
                header('Location: tampilan-xmrebate.php');
            } else {
                echo "
                <script>
                    alert('upload gagal');
